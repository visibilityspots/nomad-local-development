...
server = {{ consul__server | bool | lower }}
...
{% if not consul__server %}
retry_join = ["{{ consul__server_ip }}"]
{% endif %}
bind_addr = "{% raw %}{{{% endraw %} GetInterfaceIP \"{{ consul__interface }}\" {% raw %}}}{% endraw %}"
